...
function organizeGifts(gifts) {
  ...
  while ((match = regex.exec(gifts)) !== null) {
    ...
    giftCounts[type] = (giftCounts[type] || 0) + count;
  }

  let organizedGifts = "";

  for (const [type, count] of Object.entries(giftCounts)) {
    const palletCount = Math.floor(count / 50);
    const remaining = count % 50;

    organizedGifts += `[${type}]`.repeat(palletCount);

    const boxCount = Math.floor(remaining / 10);
    const bagCount = remaining % 10;

    organizedGifts += `{${type}}`.repeat(boxCount);
    if (bagCount > 0) organizedGifts += `(${type.repeat(bagCount)})`;
  }

  return organizedGifts;
}

console.log(organizeGifts("20a55j10k"));
